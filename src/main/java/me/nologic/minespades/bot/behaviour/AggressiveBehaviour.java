...
public class AggressiveBehaviour extends AbstractBehaviour {
    ...
    public void behave(SituationKnowledge knowledge) {
        ...
        if (knowledge.getEnemiesNear().size() > 0 && knowledge.getEnemiesNear().size() < 3) {
            ...
            if (Objects.equals(bot.getTarget(), enemyFlagCarrier)) { // FIXME: сравнение нуликов даёт true
                bot.say("Вижу носителя флага! Переключаюсь на него!");
        ...
        if (knowledge.getEnemiesNear().size() > 0 && bot.getTarget() == null) {
            ...
            bot.say(String.format("Чувствую рядом игрока %s!", closestEnemy.getName()));
        ...
        if (!bot.isBusy()) {
            ...
            if (bot.getBattlegroundPlayer().isCarryingFlag()) {
                bot.say("Я украл флаг! Несу его на базу.");
            ...
            if (enemyFlag != null && enemyFlag.getPosition() != null && bot.getBukkitPlayer().getLocation().distance(enemyFlag.getPosition()) < 20) {
                bot.say("Я вижу флаг неподалёку! Бегу поднимать!");
            ...
            if (knowledge.getDistanceToCenter() > 20) {
                ...
                bot.say("Направляюсь захватывать вражеский флаг из центра!");
                bot.moveTo(enemyFlag.getPosition());
            }
